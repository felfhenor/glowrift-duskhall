import { calculateLossyInputAmount, calculateLossyOutputAmount } from '@helpers/currency-conversion';
...
describe('Currency Conversion', () => {
  ...
  describe('calculateLossyOutputAmount', () => {
    it('should halve output for equal value currencies', () => {
      const result = calculateLossyOutputAmount(manaCurrency, manaCurrency, 100);
    ...
    it('should convert higher value to lower value currency with loss', () => {
      ...
      const result = calculateLossyOutputAmount(fireSliverCurrency, manaCurrency, 25);
    ...
    it('should convert lower value to higher value currency with loss', () => {
      ...
      const result = calculateLossyOutputAmount(manaCurrency, fireSliverCurrency, 100);
    ...
    it('should handle complex conversion ratios', () => {
      ...
      const result = calculateLossyOutputAmount(fireShardCurrency, fireSliverCurrency, 10);
      expect(result).toBe(25);
    });
  });

  describe('calculateLossyInputAmount', () => {
    it('should double input for equal value currencies', () => {
      const result = calculateLossyInputAmount(manaCurrency, manaCurrency, 50);
    ...
    it('should require double input for higher to lower value conversion', () => {
      ...
      const result = calculateLossyInputAmount(fireSliverCurrency, manaCurrency, 62);
    ...
    it('should require double input for lower to higher value conversion', () => {
      ...
      const result = calculateLossyInputAmount(manaCurrency, fireSliverCurrency, 10);
  ...
  describe('round-trip conversion prevention', () => {
    it('should prevent exact round-trip conversion', () => {
      ...
      const fireSliverAmount = calculateLossyOutputAmount(manaCurrency, fireSliverCurrency, startAmount);
      
      // Convert back from Fire Sliver to Mana
      const finalManaAmount = calculateLossyOutputAmount(fireSliverCurrency, manaCurrency, fireSliverAmount);
    ...
    it('should be consistently lossy regardless of conversion direction', () => {
      // Convert A -> B
      const outputAB = calculateLossyOutputAmount(manaCurrency, fireSliverCurrency, 100);
      
      // Calculate what input is needed for the same output
      const inputForSameOutput = calculateLossyInputAmount(manaCurrency, fireSliverCurrency, outputAB);
      ...
      const expectedInput = outputAB * 2 * (fireSliverCurrency.value / manaCurrency.value);
