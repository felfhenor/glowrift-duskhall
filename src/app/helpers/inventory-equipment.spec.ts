import { addItemToInventory, maxItemInventorySize } from '@helpers/inventory-equipment';
...
describe('Inventory Equipment Management', () => {
  ...
  function createTestItem(rarity: string, dropLevel: number, type: string = 'weapon'): EquipmentItem {
    return {
  ...
  describe('addItemToInventory when inventory is full', () => {
    beforeEach(() => {
      ...
      const maxSize = maxItemInventorySize();
    ...
    it('should have exactly max items after filling', () => {
      const inventory = mockGameState.inventory.items;
      expect(inventory).toHaveLength(maxItemInventorySize());
    });
    ...
    it('should keep new item and remove worst existing item when new item is better', () => {
      ...
      addItemToInventory(newBetterItem);
      
      const finalInventory = mockGameState.inventory.items;
      expect(finalInventory).toHaveLength(maxItemInventorySize());
      expect(finalInventory.some(i => i.id === newBetterItem.id)).toBe(true);
      
      // The worst item should be removed
      expect(finalInventory.filter(i => i.id !== newBetterItem.id)).toHaveLength(maxItemInventorySize() - 1);
    });
    ...
    it('should keep new item and remove worst existing item even when new item is worse [CURRENT BUG]', () => {
      ...
      addItemToInventory(newWorseItem);
      ...
      expect(finalInventory.some(i => i.id === worstExistingItem.id)).toBe(false);
    });
  });
